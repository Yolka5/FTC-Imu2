...
public class distanceJ extends LinearOpMode {
    ...
    private DistanceSensor distace;
    private boolean toggle;
    private  boolean result;
    Pose2d myPose = new Pose2d(-36, -60, Math.toRadians(-90));
    Pose2d startPose = new Pose2d(-36, -60, Math.toRadians(-90));
    Pose2d EndPose = new Pose2d(-39, 0, Math.toRadians(-172));
    ...
    public void runOpMode() throws InterruptedException {
        ...
        distace = hardwareMap.get(DistanceSensor.class, "distanceJ");
...
//--------------------------------------------------------------------------------------
        ...
        TrajectorySequence getToJunction = drive.trajectorySequenceBuilder(new Pose2d(-36, -60, Math.toRadians(-90)))
                .back(65)
                .lineToLinearHeading(new Pose2d(-40, -4.25, Math.toRadians(-180)))
                .turn(Math.toRadians(8.5))
...
//        drive.followTrajectory(trajectoryForward);
        ...
        while (opModeIsActive() && !isStopRequested()) {
            ...
            if (!Starting) {
                ...
                while(distace.getDistance(DistanceUnit.CM) > 25.0)
                {
                    drive.setMotorPowers(-0.17, 0, 0.17, 0);
                }
                while(distace.getDistance(DistanceUnit.CM) < 10)
                {
                    drive.setMotorPowers(0.11, 0.11, 0.11, 0.11);
                }
                while(distace.getDistance(DistanceUnit.CM) > 10)
                {
                    drive.setMotorPowers(-0.11, -0.11, -0.11, -0.11);
