...
public class AutonomRedRight extends LinearOpMode {
...
//        drive.followTrajectory(trajectoryForward);
        ...
        while (opModeIsActive() && !isStopRequested()) {
            ...
            if (!Starting) {
                ...
                if (Starting && !SawRed && colorSensor.red() < 140) {
                    telemetry.addData("driving to red", "driving to red");
                    drive.setMotorPowers(0.2, 0.2, 0.2, 0.2);
                } else if (Starting && !SawRed && colorSensor.red() >= 70) {
                    SawRed = true;
                    telemetry.addData("he saw red", "ilay pipi small");
                } else if (Starting && SawRed && !SawCancer)
                    if(distanceSensor.getDistance(DistanceUnit.CM) > 38) {
                        if ((correction * 100) <= 0) {
                            drive.setMotorPowers(0.1, 0.1, 0.1 - correction * 2, 0.1 - correction * 2);
                        } else {
                            drive.setMotorPowers(0.1 + correction * 2, 0.1 + correction * 2, 0.1, 0.1);
                        }
                    }
